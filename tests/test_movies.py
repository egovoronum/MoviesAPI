...
#         assert filter_combo["pageSize"] == 12, (
#             "PageSize filter test failed in combo filter"
#         )
        
#         assert filter_combo["page"] == 1, (
#             "Page number mismatch"
#         )
        
#         for movie in movies:
#             assert 1 <= movie["price"] <= 1000, (
#                 "Price filter test failed in combo filter"
#         )
#             assert movie["location"] == "MSK", (
#                 "Location filter test failed in combo filter"
#         )
...
#             f"Expected 400, got {response['statusCode']}"
#         )
#         assert "Поле page имеет минимальную величину 1" in response['message'], (
#             "Expected to see an error description in `message`" 
...
#             f"Expected 201, got {create_movie.status_code['statusCode']}"
#         )
...
#             f"Expected 201, got {create_response.status_code}, movie creation failed?"
#         )
#         assert patch_response.status_code == 200, (
#             f"Movie patch failed: expected status 200, got {patch_response.status_code}"
#         )
        
#         assert patch_response.json()["name"].endswith("_patched"), (
#             f"Movie name was not patched correctly: got {patch_response.json()['name']}"
#         )
        
#         assert create_response.json()["id"] == patch_response.json()["id"], (
#             "PATCH updated a different movie: created id" 
#             f"{create_response.json()['id']}, patched id {patch_response.json()['id']}"
#         )
...
#             f"Expected 404, got {response.status_code}, check PATCH movie negative"
#         )
...
#             f"Expected 200, but got {delete_response.status_code}, deletion failed??"
#         )
